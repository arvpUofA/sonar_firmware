...
void CommInterface::parseMessage(void) {
  // Check Serial for messages. Save message and set messageRecievedFlag flag to
  // true
  ...
  if (messageReceived) {
    ...
    // If commandIndex = 0 ("get") we then grab the local variable and then send
    // it
    if (commandIndex == 0) {
...
void CommInterface::getFilterPointer(Filter& _filter) { filter = &_filter; }
...
void CommInterface::getCommand(void) {
  ...
  for (uint8_t i = 0; i < NUMBER_COMMANDS; i++) {
    ...
    if (message.substring(1, 1 + commands[i].length()) == commands[i]) {
      // save the command index. This will be used to identify what the command
      // was
      commandIndex = i;
      messageReceived = true;
...
void CommInterface::getVariable(void) {
  ...
  for (uint8_t i = 0; i < NUMBER_VARIABLES; i++) {
    // if the variable length + 1 is larger than the whole message, then
    // obviously  that is not the right variable, goto the next one  Keep in
    // mind that the command portion of message was deleted at the end of the
    // getCommand method  Doing this also prevents an indexing outside of the
    // string
...
void CommInterface::getArgument() {
  ...
  // argument1length is 8 bit unsigned so if it's less than 0 it would be
  // actually be around 255  The max size of message is 32 so if we just check
  // if its larger than that then obviously  the index for a ' ' was not found
  if (argument1Length > 32) {
    // if the variable was "iSaturation", then there should have been a ' '.
    // Invalid if so.
    /*
    if( variableIndex == 4){
        messageReceived = false;
    ...
    */
    ...
    // This else is for the case of two arguments -> a space was found after
    // argument1
  }
  /*
  else {
      ...
      if ( variableIndex == 4 ){
          ...
          //Do the same as above but we're going to append the argument
  character by character argument1String = String("");
          //Keep in mind that the argument1Length is actually the index of where
  argument1 ends in message for(uint8_t i = 0; i < argument1Length ; i++){ if (
  (isDigit(message[i]) || (message[i] == '.') || (message[i] == '-') )){
                  argument1String.append( message[i] );
                  continue;
              } else {
                  //There is no tolerance here for junk characters as there is
  still another argument
                  //Thus if there are junk characters this is most definitely
  invalid messageReceived = false; argument1String = String(""); message =
  String(""); return;
              }
      ...
      //if there was space found after the argument1 and its not a variable that
  needs two arguments...
      //message was invalid
      } else {
          messageReceived = false;
...
void CommInterface::getSecondArgument(void) {
  // There should be a space at the start of message left over from getArgument
  // otherwise invalid  Remove this space as its not needed.
  if (message[0] == ' ') {
    message.remove(0, 1);
  } else {
    message = String("");
  ...
  // This is the same algorithm as in getArgument, in the case where a space is
  // not found  Because this should be the end of the message string
  argument2Length = 0;
  for (uint8_t i = 0; i < message.length(); i++) {
    if ((isDigit(message[i]) || (message[i] == '.') || (message[i] == '-'))) {
      argument2Length = i + 1;
